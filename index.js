...
const REFERRAL_BASE_URL = `https://t.me/Birrgram_bot/${MINI_APP_SHORT_NAME}`;
...
async function ensureUser(tgUser, referralId) {
  ...
  const ref = String(referralId || old.pendingReferralId || "").trim();
  ...
  const baseUser = {
    ...
    pendingReferralId: ref && /^\d+$/.test(ref) && ref !== uid ? ref : (old.pendingReferralId || null),
  ...
  if (!/^\d+$/.test(ref) || ref === uid) return;

  const referrerRef = db.collection("users").doc(ref);
  // One referral claim per invited Telegram user. This prevents duplicate rewards.
  ...
  // Credit the referrer only after the invited user has verified both mandatory channels.
  const invitedIsMember = await verifyMembership(uid);
  if (!invitedIsMember) return;

  await db.runTransaction(async tx => {
    const marker = await tx.get(claimRef);
    const referrer = await tx.get(referrerRef);
    if (marker.exists || !referrer.exists) return;
  ...
  // Referral has been consumed successfully; clear the pending value.
  await userRef.set({ pendingReferralId: admin.firestore.FieldValue.delete() }, { merge: true });
}
...
app.post("/api/verify-channels", async (req, res) => {
  try {
    ...
    if (verified) await ensureUser(u, req.body.referralId);
    res.json({ verified });
  } catch (e) {
    sendErr(res, e);
  }
...
app.post("/api/user", async (req, res) => {
  try {
    ...
    await ensureUser(u, req.body.referralId);
...
app.post("/api/withdraw", async (req, res) => {
  try {
    ...
    await db.runTransaction(async tx => {
      ...
      if (!snap.exists) throw new Error("User not found");
      const d = snap.data();
      if (Number(d.points || 0) < WITHDRAW_POINTS) throw new Error("20,000 ነጥብ ሳይደርስ ማውጣት አይችሉም።");
