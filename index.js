...
app.post("/telegram/webhook", async (req, res) => {
  // Acknowledge Telegram immediately; process the update asynchronously.
  res.sendStatus(200);
  try {
      ...
      if (/^\/start(?:@\w+)?(?:\s+.*)?$/i.test(text)) {
        ...
        await bot("sendMessage", {
          ...
          reply_markup: {
            inline_keyboard: [[{
              text: "🚀 Birr Gram ክፈት",
              url: `${REFERRAL_BASE_URL}${validRef ? `?startapp=${encodeURIComponent(startPayload)}` : "?startapp=" + encodeURIComponent(uid)}`
            }]]
          }
        });
        return;
