  ...
  try {
    ...
    if (message?.text) {
      ...
      if (/^\/start(?:@\w+)?(?:\s+.*)?$/i.test(text)) {
        ...
        const validRef = /^\d+$/.test(startPayload) && String(user.id) !== startPayload;
        if (validRef) {
        ...
        const webAppUrl = process.env.WEBAPP_URL || "https://birr-gram.vercel.app";
        await bot("sendMessage", {
          chat_id: message.chat.id,
          text: `👋 <b>እንኳን ወደ Birr Gram በደህና መጡ!</b>\n\n🎯 ማስታወቂያ በመመልከት፣ ጓደኞችን በመጋበዝ እና ዕለታዊ ሽልማት በመውሰድ ነጥብ ያግኙ።\n\n👇 <b>የBirr Gram ዌብ አፕን ለመክፈት</b>`,
          parse_mode: "HTML",
          reply_markup: { inline_keyboard: [[{ text: "🚀 Birr Gram ክፈት", web_app: { url: webAppUrl } }]] }
        });
