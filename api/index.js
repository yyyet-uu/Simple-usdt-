...
async function handleChannelCheck(req, res, body) {
  ...
  const requestedChannel = cleanString(body.channel, 100);
  const channel =
    requestedChannel === "1" || requestedChannel === CONFIG.channels.channel1
      ? CONFIG.channels.channel1
      : requestedChannel === "2" || requestedChannel === CONFIG.channels.channel2
      ? CONFIG.channels.channel2
      : "";
...
async function router(req, res) {
  ...
  // Friendly fallback: if the Telegram Mini App URL was accidentally set
  // to /api/user, show the app instead of the confusing 405 response.
  // The real API still uses POST /api/user.
  if (req.method === "GET") {
    const parsedGetUrl = new URL(
      rawUrl,
      `https://${req.headers.host || "localhost"}`
    );
    if (parsedGetUrl.pathname === "/api/user") {
      try {
        const html = fs.readFileSync(
          nodePath.join(__dirname, "index.html"),
          "utf8"
        );
    ...
    return json(res, 405, {
      ok: false,
      error: "Method not allowed. Use the Telegram Web App URL at the site root.",
    });
  }
