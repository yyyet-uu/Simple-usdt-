...
function getTelegramInitData(req, body) {
  ...
  return cleanString(header || body.telegramInitData || body.initData, 10000);
}
...
function normalizeUserRecord(data, telegramUser) {
  ...
  return {
    ...
    referralLink:
      data?.referralLink ||
      `https://t.me/${CONFIG.botUsername}?startapp=ref_${String(
        telegramUser.id
      )}`,
...
function requirementsMet(data) {
  ...
  return (
    ...
    Boolean(cycle.channel1) &&
    Boolean(cycle.channel2)
  );
}
...
async function validateReferralAfterChannels(
  db,
  referredTelegramId
) {
  const referredRef = userRef(db, referredTelegramId);
  const referredSnap = await referredRef.once("value");
  const referred = referredSnap.val() || {};
  const referral = referred.referral;

  if (!referral?.referrerId || referral.valid === true) {
    return { becameValid: false, referrerId: referral?.referrerId || null };
  }

  const membership = await verifyBothChannels(referredTelegramId);
  if (!membership.channel1 || !membership.channel2) {
    return { becameValid: false, referrerId: referral.referrerId, membership };
  }

  // Atomically claim the referral so two channel-check requests cannot
  // increment the referrer twice.
  let claimed = false;
  await referredRef.child("referral").transaction((current) => {
    if (!current || !current.referrerId || current.valid === true) return current;
    claimed = true;
    return {
      ...current,
      valid: true,
  ...
  await referrerRef.child("cycle").transaction((cycle) => {
    if (!cycle) return cycle;
    const referrals = Number(cycle.referrals || 0);
    return {
      ...cycle,
      referrals: Math.min(referrals + 1, CONFIG.requirements.referrals),
    };
  });

  await referredRef.child("updatedAt").set(admin.database.ServerValue.TIMESTAMP);
...
async function handleUser(req, res, body) {
  ...
  const referralLink =
    ...
    `https://t.me/${CONFIG.botUsername}?startapp=${referralCode}`;
...
async function handleChannelCheck(req, res, body) {
  ...
  let channel = cleanString(body.channel, 100);

  // The frontend can send 1/2; map those to the configured Telegram
  // channel usernames before calling getChatMember.
  if (channel === "1") channel = CONFIG.channels.channel1;
  if (channel === "2") channel = CONFIG.channels.channel2;
  ...
  if (joined) {
    ...
    // have been verified.
    await validateReferralAfterChannels(db, id);
  }
...
async function handleWithdraw(req, res, body) {
  ...
  if (!locked) {
    ...
    throw new Error("Withdrawal is not available yet. Complete all requirements first.");
  }
...
async function router(req, res) {
  ...
  const forwardedPath = req.query?.path || req.query?.route || "";
  const path = String(
