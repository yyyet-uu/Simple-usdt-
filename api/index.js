...
async function validateReferralAfterChannels(
  ...
  const referredSnap = await userRef(
    db,
    referredTelegramId
  ).once("value");
  ...
  const membership = await verifyBothChannels(
    referredTelegramId
  );
  ...
  const result = await referrerRef.transaction((current) => {
    if (!current) return current;

    const cycle = current.cycle || {};
    const referrals = Number(cycle.referrals || 0);

    if (referral.valid === true) {
      return current;
    }
  ...
  await userRef(db, referredTelegramId)
    .child("referral")
    .update({
      valid: true,
