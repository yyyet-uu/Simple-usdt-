...
const BALANCE_RESET_VERSION = 2; // one-time reset of existing user point balances
const WITHDRAW_BIRR = 30;
const REFERRALS_PER_WITHDRAWAL = 6;
...
async function verifyMembership(uid) {
  for (const chat of CHANNELS) {
    try {
      ...
      throw new Error(`የ${chat} አባልነት ማረጋገጥ አልተቻለም። Telegram: ${e.message || "unknown error"}`);
    }
  }
  return true;
}
...
async function ensureUser(tgUser, referralId) {
  ...
  // Legacy migration intentionally disabled. Older users use safe defaults from buildState.
  // This prevents expensive taskClaims/withdrawals reads on every verification request.
...
async function getState(uid) {
  ...
  let data = snap.data();
  if (Number(data.balanceResetVersion || 0) < BALANCE_RESET_VERSION) {
    await getDb().runTransaction(async tx => {
      const fresh = await tx.get(userRef);
      if (!fresh.exists) throw new Error("User not found");
      const freshData = fresh.data();
      if (Number(freshData.balanceResetVersion || 0) < BALANCE_RESET_VERSION) {
        tx.update(userRef, {
          points: 0,
          balanceResetVersion: BALANCE_RESET_VERSION,
          updatedAt: admin.firestore.FieldValue.serverTimestamp()
        });
        data = { ...freshData, points: 0, balanceResetVersion: BALANCE_RESET_VERSION };
      } else {
        data = freshData;
      }
    });
  }
  return buildState(uid, data);
}
...
app.post("/api/ad-complete", async (req, res) => {
  try {
    ...
    await getDb().runTransaction(async tx => {
      ...
      if (count >= DAILY_AD_LIMIT) throw new Error("የዛሬ 100 ማስታወቂያ ገደብ ደርሷል።");
