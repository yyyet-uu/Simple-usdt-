...
const CHANNELS = ["@usdt_g_ram", "@usdt_hub_payment_proof", "@halalincomes1", "@UltraProfitSpot", "@BEKI_TECH7"];
const REWARD_TASKS = [
  ...
  { id: "funnyvide1_youtube", title: "Funny Video YouTube", username: "@funnyvide1-j7", url: "https://youtube.com/@funnyvide1-j7", points: 500, type: "youtube" }
];
...
const REFERRALS_PER_WITHDRAWAL = 3;

// Fixed promotional codes. Each code can be claimed once per user and gives 3,000 points.
...
// Initialize Firebase lazily at runtime. This prevents Vercel's build step from
// failing when environment variables are not injected into the build process.
let db = null;
function getDb() {
  if (db) return db;
...
async function ensureUser(tgUser, referralId) {
  ...
  // One-time migration for older users created by previous Birr Gram builds.
  // New users already receive these fields below, so normal app opens do not
  // query taskClaims/withdrawals anymore.
  if (existing.exists && (old.claimedTasks === undefined || old.withdrawalCount === undefined || old.withdrawalHistory === undefined)) {
    const [taskSnaps, withdrawalSnap] = await Promise.all([
      Promise.all(REWARD_TASKS.map(task => getDb().collection("taskClaims").doc(`${uid}_${task.id}`).get())),
      getDb().collection("withdrawals").where("userId", "==", uid).get().catch(() => null)
    ]);
    const claimedTasks = taskSnaps.filter(x => x.exists).map(x => x.data().taskId);
    const withdrawalHistory = withdrawalSnap ? withdrawalSnap.docs.map(x => {
      const w = x.data();
      return { id: x.id, firstName: w.firstName || "", telebirr: w.telebirr || "", points: Number(w.points || WITHDRAW_POINTS), amountBirr: Number(w.amountBirr || WITHDRAW_BIRR), status: w.status || "PENDING", createdAt: w.createdAt?.toDate?.()?.toISOString() || new Date().toISOString() };
    }).sort((a,b) => String(b.createdAt).localeCompare(String(a.createdAt))) : [];
    const pending = withdrawalHistory.find(x => x.status === "PENDING");
    await userRef.update({
      claimedTasks: claimedTasks.length ? claimedTasks : [],
      withdrawalCount: withdrawalHistory.length,
      withdrawalHistory,
      pendingWithdrawalId: pending ? pending.id : null,
      updatedAt: admin.firestore.FieldValue.serverTimestamp()
    });
    old.claimedTasks = claimedTasks;
    old.withdrawalCount = withdrawalHistory.length;
    old.withdrawalHistory = withdrawalHistory;
    old.pendingWithdrawalId = pending ? pending.id : null;
  }

  // Avoid a Firestore write on every app open. Only initialize a new user or
  // store a genuinely new referral/profile value.
  if (!existing.exists) {
    await userRef.set({
      userId: uid,
      ...
      withdrawalCount: 0,
      withdrawalHistory: [],
      pendingReferralId: validRef ? ref : null,
      lastWithdrawalDate: null,
      createdAt: admin.firestore.FieldValue.serverTimestamp(),
      updatedAt: admin.firestore.FieldValue.serverTimestamp()
    });
  } else if (validRef && String(old.pendingReferralId || "") !== ref) {
    await userRef.update({
      pendingReferralId: ref,
      updatedAt: admin.firestore.FieldValue.serverTimestamp()
    });
  }
  ...
  const referrerRef = getDb().collection("users").doc(ref);
  const claimRef = getDb().collection("referralClaims").doc(uid);
  let credited = false;
...
function buildState(uid, d) {
  const date = todayKey();
  const adsToday = d.lastAdDate === date ? Number(d.adsToday || 0) : 0;
  const claimedToday = d.lastClaimDate === date;
  const history = Array.isArray(d.withdrawalHistory) ? d.withdrawalHistory : [];
  const claimedTasks = new Set(Array.isArray(d.claimedTasks) ? d.claimedTasks.map(String) : []);
  return {
    userId: String(uid), firstName: d.firstName || "",
    points: Number(d.points || 0), referrals: Number(d.referrals || 0),
    adsToday, remainingAds: Math.max(0, DAILY_AD_LIMIT - adsToday),
    claimedToday, claimStreak: Number(d.claimStreak || 0),
    dailyClaimPoints: DAILY_CLAIM_POINTS, referralReward: POINTS_PER_REF,
    referralLink: `${REFERRAL_BASE_URL}?startapp=${encodeURIComponent(uid)}`,
    withdrawPoints: WITHDRAW_POINTS, withdrawBirr: WITHDRAW_BIRR,
    withdrawalCount: Number(d.withdrawalCount || history.length),
    referralsPerWithdrawal: REFERRALS_PER_WITHDRAWAL,
    lastWithdrawalDate: d.lastWithdrawalDate || null,
    withdrawals: history,
    rewardTasks: REWARD_TASKS.filter(task => !claimedTasks.has(String(task.id)))
  };
}
...
async function getState(uid) {
  ...
  return buildState(uid, snap.data());
}

async function claimPromoCode(uid, rawCode) {
  const code = String(rawCode || "").trim().toUpperCase();
  const reward = PROMO_CODES.get(code);
  if (!reward) throw new Error("❌ የPromo Code ስህተት ነው ወይም አይገኝም።");

  const promoRef = getDb().collection("promoClaims").doc(`${String(uid)}_${code}`);
  const userRef = getDb().collection("users").doc(String(uid));
  let credited = false;

  await getDb().runTransaction(async tx => {
    const [promoSnap, userSnap] = await Promise.all([tx.get(promoRef), tx.get(userRef)]);
    if (!userSnap.exists) throw new Error("User not found");
    if (promoSnap.exists) throw new Error("❌ ይህ Promo Code ቀድሞ ተጠቅመውበታል።");
...
app.post("/api/reward-tasks", async (req, res) => {
  try {
    ...
    res.json({ tasks: (await getState(u.id)).rewardTasks });
...
app.post("/api/claim-task", async (req, res) => {
  try {
    ...
    const result = await claimRewardTask(u.id, taskId);
    const state = result.stateData ? buildState(u.id, result.stateData) : await getState(u.id);
    res.json({ credited: result.credited, state });
  } catch (e) { sendErr(res, e); }
});
...
app.post("/api/verify-channels", async (req, res) => {
  try {
    ...
    res.json({ verified, referralCredited, state: verified ? await getState(u.id) : null });
  } catch (e) {
    sendErr(res, e);
  }
});
...
app.post("/api/user", async (req, res) => {
  try {
    ...
    if(/^\d+$/.test(incoming) && incoming !== String(u.id)){
      await getDb().collection("users").doc(String(u.id)).set({
...
app.post("/api/claim-promo", async (req, res) => {
  try {
    ...
    const credited = await claimPromoCode(u.id, code);
    res.json({ credited, state: await getState(u.id) });
  } catch (e) { sendErr(res, e); }
});
...
app.post("/api/daily-claim", async (req, res) => {
  try {
    const u = await requireVerified(req);
    const ref = getDb().collection("users").doc(String(u.id));
    const date = todayKey();
    let streak = 1;
    ...
    await getDb().runTransaction(async tx => {
      ...
      updatedData = { ...d, points: Number(d.points || 0) + DAILY_CLAIM_POINTS, lastClaimDate: date, claimStreak: nextStreak };
    });

    res.json({ state: updatedData ? buildState(u.id, updatedData) : await getState(u.id), streak });
  } catch (e) {
    sendErr(res, e);
  }
});
...
app.post("/api/ad-complete", async (req, res) => {
  try {
    ...
    await getDb().runTransaction(async tx => {
      ...
      updatedData = { ...d, points: Number(d.points || 0) + POINTS_PER_AD, adsToday: count + 1, lastAdDate: date };
    });

    res.json({ state: updatedData ? buildState(u.id, updatedData) : await getState(u.id) });
  } catch (e) {
    sendErr(res, e);
  }
});
...
app.post("/api/withdraw", async (req, res) => {
  try {
    ...
    let withdrawalId;
    await getDb().runTransaction(async tx => {
      const userRef = getDb().collection("users").doc(String(u.id));
      const snap = await tx.get(userRef);
      if (!snap.exists) throw new Error("User not found");
      const d = snap.data();
      if (Number(d.points || 0) < WITHDRAW_POINTS) throw new Error("30,000 ነጥብ ሳይደርስ ማውጣት አይችሉም።");

      // Keep the withdrawal count on the user document so every withdrawal
      // request needs only the single user-document transaction read.
      const previousWithdrawalCount = Number(d.withdrawalCount || 0);
      const requiredReferrals = (previousWithdrawalCount + 1) * REFERRALS_PER_WITHDRAWAL;
      if (Number(d.referrals || 0) < requiredReferrals) {
        throw new Error(`ለዚህ ማውጣት ${requiredReferrals} ሪፈራል ያስፈልገዋል።`);
      }

      // Only one withdrawal request is allowed per Addis Ababa calendar day.
      const today = todayKey();
      if (d.lastWithdrawalDate === today) {
        throw new Error("በቀን አንድ ጊዜ ብቻ ማውጣት ይችላሉ። ነገ እንደገና ይሞክሩ።");
      }

      if (d.pendingWithdrawalId) throw new Error("አንድ የሚጠባበቅ የክፍያ ጥያቄ አለዎት።");

      const wr = getDb().collection("withdrawals").doc();
      withdrawalId = wr.id;
      const createdAtIso = new Date().toISOString();
      const historyEntry = {
        id: wr.id, firstName, telebirr, points: WITHDRAW_POINTS,
        amountBirr: WITHDRAW_BIRR, status: "PENDING", createdAt: createdAtIso
      };
      tx.update(userRef, {
        points: admin.firestore.FieldValue.increment(-WITHDRAW_POINTS),
        lastWithdrawalDate: today,
        withdrawalCount: previousWithdrawalCount + 1,
        pendingWithdrawalId: wr.id,
        withdrawalHistory: admin.firestore.FieldValue.arrayUnion(historyEntry),
        updatedAt: admin.firestore.FieldValue.serverTimestamp()
      });
      tx.set(wr, {
    ...
    await getDb().collection("withdrawals").doc(withdrawalId).update({
...
app.post("/telegram/webhook", async (req, res) => {
  ...
  try {
    ...
    if (message?.text) {
      ...
      if (/^\/start(?:@\w+)?(?:\s+.*)?$/i.test(text)) {
        ...
        if (updateId !== undefined && updateId !== null) {
          try {
            await getDb().collection("telegramUpdates").doc(String(updateId)).create({
    ...
    if (cb) {
      ...
      const wr = getDb().collection("withdrawals").doc(id);
      ...
      if (d.status !== "SUCCESSFULLY") {
        ...
        const userRef = getDb().collection("users").doc(String(d.userId));
        const us = await userRef.get();
        if (us.exists) {
          const ud = us.data();
          const history = Array.isArray(ud.withdrawalHistory) ? ud.withdrawalHistory : [];
          const updatedHistory = history.map(item => item.id === id ? { ...item, status: "SUCCESSFULLY" } : item);
          await userRef.update({
            withdrawalHistory: updatedHistory,
            pendingWithdrawalId: ud.pendingWithdrawalId === id ? null : (ud.pendingWithdrawalId || null),
            updatedAt: admin.firestore.FieldValue.serverTimestamp()
          });
        }
      }
